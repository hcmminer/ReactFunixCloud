...
export const staffsPageSlice = createSlice({
	...
	reducers: {
		...
		staffsSearched: (state, action) => {
			const originState = structuredClone(state.staffs)
			console.log("🚀 => file: staffsPageRedux.js => line 15 => originState", originState)
			if (action.payload === '') {
				state.staffs = originState
			} else {
				const newState = originState.staffs.filter((item) => {
					item.name
						.toLowerCase()
