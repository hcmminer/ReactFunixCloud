...
const HeaderComponent = ({ setScreen }) => {
    ...
    console.log("🚀 => file: HeaderComponent.jsx => line 12 => HeaderComponent => sortBy", sortBy)
     // sortBy cua cpm va trong ham EVENT DOM khac nhau
    const dispatch = useDispatch();
    const [currentSearchInput, setCurrentSearchInput] = useState("");
    const location = useLocation();
    const activeLink = location.pathname;
    useEffect(() => {
        // khi state thay doi (cua redux) thi chi thanh phan DOM lien quan moi render lai
        dispatch(staffsSortByChanged(sortBy));
        console.log(sortBy);
    }, []);
    let rightMenu;
            ...
            <div className="sm:col-span-2 hidden md:flex items-center justify-end">
                <select
                    onChange={(e) => {
                        ...
                        // trong event DOM thì chạy xong event mới thay đổi biến (sortBy)
                        console.log(sortBy); // khi thay đổi thì sortBy vẫn mang giá trị trước khi chạy hàm onChange
                    }}
                    value={sortBy}
