...
const NewStaff = ({
    ...
    const FormikApp = withFormik({
        mapPropsToValues({
            ...
            if (idStaffEdit) {
                return {
                    id: currentStaffEdit.id,
                    name: currentStaffEdit.name,
                    doB: dateFormat(currentStaffEdit.doB, "yyyy-mm-dd"),
                    salaryScale: currentStaffEdit.salaryScale,
                    startDate: dateFormat(currentStaffEdit.doB, "yyyy-mm-dd"),
                    departmentId: currentStaffEdit.departmentId,
                    annualLeave: currentStaffEdit.annualLeave,
                    overTime: currentStaffEdit.overTime,
                };
            } else {
                return {
                    name: name || "",
        ...
        handleSubmit(values, { resetForm, setErrors, setSubmitting }) {
            setTimeout(() => {
                ...
                const patchStaff = async () => {
                    try {
                        const res = await publicRequest.patch("staffs", {
                            ...values,
                            image: "/assets/images/alberto.png",
                        });
                        console.log(
                            "🚀 => file: NewStaff.jsx => line 280 => patchStaff => res",
                            res
                        );
